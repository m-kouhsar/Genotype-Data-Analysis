...
mkdir -p Ethnicity 
echo
echo "change variant ids to chr:bp in input data"
echo
...
echo
echo "change variant ids to chr:bp in reference data"
echo
...
echo
echo "Merging the data with ref genome"
echo
plink --bfile Ethnicity/${FilePrefix}_QC_final_1kgIDs --bmerge $RefGenome_binary --maf 0.1 --geno 0.1 --make-bed --out Ethnicity/${FilePrefix}_QC_final_mergedw1000G

if [ -e Ethnicity/${FilePrefix}_QC_final_mergedw1000G*.missnp ]
then
  echo
  echo "Removing problematic SNPs from the data..."
  echo
