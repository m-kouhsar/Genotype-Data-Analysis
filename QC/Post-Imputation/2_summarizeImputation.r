...
for(chr in 1:22){
	png(paste(OutDir,"/ImputationQualityPlots_chr", chr,"_",colName, ".png", sep = ""), width = 12, height = 8, units = "in", res = 200)
	par(mfrow = c(2,2))
...
write.csv(nVar, paste(OutDir,"/",colName,"_nVariantsSummary.csv", sep = ""))
write.csv(compRefPanel, paste(OutDir,"/",colName,"_CompRefPanelSummary.csv", sep = "")) 
