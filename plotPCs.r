...
#args <- c("/lustre/projects/Research_Project-191391/Genotyping/Imputation_Results/ImputationOutput","NIH37","3")

args<-commandArgs(TRUE)

data_dir<-args[1]
FilePrefix <- args[2]
nSD<-as.numeric(args[3])

setwd(data_dir)
#print(data_dir)
#print(FilePrefix)
#print(nSD)
impute<-read.table(paste0(FilePrefix,"_QCd.pca.eigenvec"))

pcMu<-colMeans(impute[,-c(1:2)])
pcSigma<-apply(impute[,-c(1:2)], 2, sd)

pdf(paste0(FilePrefix,"_IputationScatterplotPCs.pdf"), width = 12, height = 4)
par(mfrow = c(1,3))
par(mar = c(4,4,1,1))
...
write.table(outliers, paste(FilePrefix,"_OutliersFromPC_",nSD,"SDfromMean.txt", sep = ""), quote = FALSE, sep = " ")
