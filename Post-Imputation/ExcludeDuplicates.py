...
print(f'Reading in sample missingness from {sys.argv[2]}')

sampleMissing = open(sys.argv[2], "r")
...
print(f"Missingness info read for {str(len(sMissing))} samples")	

print(f'Identifying duplicates from1 {sys.argv[1]}')

dupSamplesFile = open(sys.argv[1], "r")
...
print(f"{str(len(uniqList))} groups of duplicate samples")
uniqList.sort()


print(f'Writing list of samples to exclude to {sys.argv[3]}')
...
for item in uniqList:
	...
	for element in samples:
		if element in sMissing:
			...
			print(f"Couldn't find sample: {element}")
